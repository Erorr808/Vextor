...
namespace Unity.FPS.Gameplay
{
    ...
    public class PlayerWeaponsManager : MonoBehaviour
    {
        ...
        [Header("Start-of-Match Loadout")]
        [Tooltip("If enabled, the player chooses a starting weapon before the match begins")]
        public bool EnableWeaponSelectionAtStart = false;

        [Tooltip("Ammo each starting weapon carries")]
        public int StartingMaxAmmo = 999999999;

        [Tooltip("Reload duration for each starting weapon")]
        public float StartingReloadSeconds = 3.7f;

        [Tooltip("Weapons available for selection at match start")]
        public List<WeaponController> SelectableWeaponPrefabs = new List<WeaponController>();
        ...
        List<WeaponController> m_AvailableWeapons = new List<WeaponController>();
        ...
        void PrepareSelectableWeapons()
        {
            m_AvailableWeapons.Clear();

            foreach (var weapon in StartingWeapons)
            {
                AddSelectableWeapon(weapon);
            }

            foreach (var weapon in SelectableWeaponPrefabs)
            {
                AddSelectableWeapon(weapon);
            }
            ...
            {
                foreach (var weapon in Resources.FindObjectsOfTypeAll<WeaponController>())
                {
                    if (weapon != null && weapon.name.Contains("Weapon_") && !m_AvailableWeapons.Contains(weapon))
                    {
        ...
        void AddSelectableWeapon(WeaponController weapon)
        {
            if (weapon == null || m_AvailableWeapons.Contains(weapon))
                return;
        ...
        {
            foreach (var weaponPrefab in m_AvailableWeapons)
            {
                if (weaponPrefab != null && HasWeapon(weaponPrefab) == null)
                {
                    AddWeapon(weaponPrefab);
                }
            }

            if (GetActiveWeapon() == null && m_AvailableWeapons.Count > 0)
            {
                SwitchWeapon(true);
            }
        }

        int GetWeaponSlotIndexForPrefab(WeaponController weaponPrefab)
        {
            for (int i = 0; i < m_WeaponSlots.Length; i++)
            {
                if (m_WeaponSlots[i] != null && m_WeaponSlots[i].SourcePrefab == weaponPrefab.gameObject)
                {
                    return i;
                }
            }
        ...
        public bool AddWeapon(WeaponController weaponPrefab)
        {
            ...
            for (int i = 0; i < m_WeaponSlots.Length; i++)
            {
                ...
                if (m_WeaponSlots[i] == null)
                {
                    ...
                    weaponInstance.ConfigureForStartLoadout(StartingMaxAmmo, StartingReloadSeconds);
